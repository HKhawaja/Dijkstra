...
{
    /**
     * Maps items to indexes in the itemTable array.
     */
     ...
     * basically your extractMin
     * @return the item with the lowest priority
     */
    public K removeItem()
    {
    //get the HeapEntry and the item itself		
	HeapEntry<V> heapTop = itemHeap.get(0);
	K result = itemTable.get(heapTop.tableIndex).item;

	// move last element to top of heap and remove old top
	swap(0, size - 1);
    ...
    //this is basically your decreaseKey
	...
	if (itemMap.containsKey(item))
	    ...
	    {
		//do I really want to re-add the item if it wasn't present in the priority queue?
		//nope
